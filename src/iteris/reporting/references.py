...
def build_bibtex_entries(evidence: dict[str, Any], *, version: str = "") -> list[BibTeXEntry]:
    report_id = str(evidence.get("report_id") or "")
    version = version or str(evidence.get("version") or "")
    ...
    evidence_path = _evidence_path(report_id, version)
    _append_entry(
        entries,
        seen,
        kind="evidence",
        ...
        title=f"Evidence register for {report_id or 'report'}",
    ...
    for role, request_id in _verification_ids(evidence, answer):
        ...
        _append_entry(
            ...
            title=f"Verification record: {role}",
            howpublished=f"Project file: {path}",
    ...
    for idx, fact in enumerate(facts, 1):
        ...
        _append_entry(
            ...
            title=f"Fact F{idx}: {fact.get('claim_summary') or fact_id}",
            howpublished=f"Project file: {path}",
...
def render_bibtex(value: dict[str, Any] | list[BibTeXEntry]) -> str:
    ...
    if not entries:
        return "% No internal references emitted in portable mode.\n"
    chunks = ["% Project file paths are relative."]
...
def _evidence_path(report_id: str, version: str = "") -> str:
