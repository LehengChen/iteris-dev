"""Render report intermediate data to an amsart LaTeX source file."""
...
from iteris.reporting.templates import template_rendering
...
def render_report(
    ...
    *,
    ...
    return (
        "% Generated by iteris report. Project file references are relative paths.\n"
        f"% Primary source artifact: {target_artifact or 'unknown'}\n"
        f"{_preamble(report)}"
        f"{_begin_document(report, abstract)}"
        f"{body}\n"
...
def _preamble(report: dict[str, Any]) -> str:
    title = latex_escape(report.get("title") or "Iteris Report")
    template_id = str(report.get("template") or "amsart")
    rendering = template_rendering(template_id)
    if rendering.get("topmatter") == "siamart":
        options = _document_options(rendering)
        return (
            f"\\documentclass{options}{{{rendering.get('document_class') or 'siamart'}}}\n"
            ...
            "\\newcommand{\\TheAuthors}{Iteris Report Draft}\n"
            "\\headers{\\TheTitle}{\\TheAuthors}\n"
            "\\title{\\TheTitle}\n"
            "\\author{Iteris Report Draft}\n"
        )
    return (
...
def _begin_document(report: dict[str, Any], abstract: str) -> str:
    if template_rendering(str(report.get("template") or "amsart")).get("topmatter") == "siamart":
        return (
            "\\begin{document}\n"
            "\\maketitle\n\n"
            "\\begin{abstract}\n"
            ...
            "\\end{abstract}\n\n"
            "\\begin{keywords}\n"
            "Gaussian elimination with complete pivoting, fermionic kernel, cross approximation, total positivity, Fekete determinants\n"
            "\\end{keywords}\n\n"
            "\\begin{AMS}\n"
            "15A23, 15B48, 41A20, 65F55\n"
            "\\end{AMS}\n\n"
        )
    return (
        "\\begin{document}\n"
...
def _abstract_text(evidence: dict[str, Any]) -> str:
    ...
    if result:
        return (
            "This Iteris-generated draft records a verified result and its evidence trace. "
            f"The verified positive result is: {result}"
        )
    return "This Iteris-generated draft records the current verified result and its evidence trace."
...
def _introduction_text(
    ...
    *,
    ...
    body = [
        "This draft is generated from an Iteris project after the answer-verification gate"
        + cite
        + ". "
        "The mathematical text should be edited by the author, while the citation registry keeps the draft tied to the fact graph.",
    ]
    if result:
...
def _theorem_text(
    ...
    *,
    ...
    text = latex_escape(result or "The verified Iteris target artifact establishes the stated project result")
    return text + cite + "."
...
def _evidence_register_note(evidence: dict[str, Any], references: dict[str, Any]) -> str:
    ...
    return (
        ...
        "Detailed Iteris evidence is stored in "
...
def _author_draft_parts(
    ...
    *,
    ...
    abstract = _apply_citations(markdownish_to_latex(abstract_md), references, include_references=include_references).strip()
...
def _bibliography_block(report: dict[str, Any], include_references: bool) -> str:
    if not include_references:
        return ""
    style = template_rendering(str(report.get("template") or "amsart")).get("bibliography_style") or "plain"
